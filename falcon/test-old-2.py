...
class ExternalPublicKey(falcon.SecretKey):
	...
	def __init__(self, pubkey):
		...
		# Convert from Base64
		pubkey = base64Decode(pubkey.encode())
		pubkey = 
		# Extract logn & n
		logn = (pubkey[0] & 0x0F)
		n = 2 ** logn
		# Extract h
		self.h = []
		v = modq_decode(self.h, logn, pubkey[1:])
		if v == 0:
	...
	def verifyBase64(self, message, signature):
		signature = base64Decode(signature.encode())
...
print("------------------------------------")
print("privkey")
privkey = base64Decode(privkey.encode())
